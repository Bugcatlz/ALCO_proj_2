...
int findRegIndex(Register* r, vector <Register*> registers);//當register的位置為r時，找到其在regiseter中的位置
...
int main(void)
{
	...
	if (num <= 0 || num%2==1 ) //不為奇數且大於0
	{
		cout << "\nNumber of entries required exceeds zero!" << endl;
		system("pause");
		exit(1);
	}
	...
	while (i < input.size())
	{
		...
		else
		{
			if (i != input.size() - 1)//判斷是否不為最後一個label
			{
				i++;
				insts.push_back(converInst(input[i]));
				string n = converLabel(input[i - 1]);//name
				Label temp(n, insts[insts.size() - 1].getAddress());
				labs.push_back(temp);
			}
			else
			{
